...
public:
    ...
    Solver() {
        ...
        //File reading cannot be parallelized
        for (int j = 0; j < n[1]; j++) {
        ...
        //collapse(2) for two loops nested loops
       #pragma omp parallel for collapse(2)
        for (int j = 0; j < n[1]; j++) {
            for (int i = 0; i < n[0]; i++) {
                int idx = i * STRIDE_I + j;  //Calculate inside
                p1[idx] = 0.0;
                q1[idx] = 0.0;
            }
        }
        ...
        time = 0;
            ...
            for (int i = 0; i < n[0]; i++) {
                ...
                for (int j = 0; j < n[1]; j++) {
                    ...
                    u[idx] = uinf;
                    u[STRIDE_K + idx] = vinf;
                    u[2*STRIDE_K + idx] = 0.0;
                    
                    up[idx] = uinf;
                    up[STRIDE_K + idx] = vinf;
        ...
        #pragma omp parallel 
        {
            ...
            for(int k=0; k<2; k++){
                ...
                for(int i = 0; i < n[0]; i++) {
                    int uk_base = k * STRIDE_K + i * STRIDE_I + j;  // Calculate for each i
        ...
        // cout << "Setting boundary conditions at infinity..." << endl;
            ...
            for(int i = 0; i < n[0]-1; i++) {
                ...
                double vnn = u[u0_base]*xnox[i] + u[u1_base]*xnoy[i];
            ...
            //Handle periodic boundary separately (after parallel region)
            // This avoids race condition from i==0 special case
            int u0_base_0 = j;  // i=0
            int u1_base_0 = STRIDE_K + j;
            int u2_base_0 = 2*STRIDE_K + j;
            int last_idx = (n[0]-1) * STRIDE_I + j;  // i=n[0]-1

            u[last_idx] = u[u0_base_0];
            u[last_idx + STRIDE_K] = u[u1_base_0];
    ...
    void timeLoop(){
        ...
        // ========================================
            ...
            for(int i = 0; i < n[0]; i++) {
                for(int j = 0; j < n[1]; j++) {
                    int idx = i * STRIDE_I + j;
                    sol[idx] = u[idx + STRIDE_K];  // u[1][i][j]
                }
            }
            ...
            for(int i = 0; i < n[0]; i++) {
                for(int j = 0; j < n[1]; j++) {
                    int idx = i * STRIDE_I + j;
                    sol[idx] = u[idx + 2*STRIDE_K];  // u[2][i][j]
                }
            }
            ...
            // done parallel but dependency is present in this loop
            #pragma omp parallel for schedule(static)
            for(int i = 0; i < n[0] - 1; i++) {
                int j = n[1] - 1;
            ...
            for(int i = 0; i < n[0]; i++) {
                for(int j = 0; j < n[1]; j++) {
                    int idx = i * STRIDE_I + j;
                    uold[idx] = u[idx];                    // u[0][i][j]
                    uold[idx + STRIDE_K] = u[idx + STRIDE_K];  // u[1][i][j]
                }
            }
            ...
            for (int i = 0; i < n[0]; i++) {
                for (int j = 0; j < n[1]; j++) {
                    ...
                    uxi[idx] = dxix[idx] * u[u0] + dxiy[idx] * u[u1];
                    uet[idx] =  dex[idx] * u[u0] +  dey[idx] * u[u1];
                }
            }
            // at solid boundary
            ...
            for (int i = 0; i < n[0] - 1; i++) {
                ...
                for (int k = 0; k < 2; k++) {
                    ...
                    double gg  = gamma[idx] * (u[uk_i_jpp+1] + u[uk_ij] - 2.0*u[uk_i_jpp]) / (dxi[1]*dxi[1]);
                    double bb  = beta[idx] * (u[ipp*STRIDE_I + jpp + k*STRIDE_K]
                                            + u[uk_inn_j]
                                            - u[inn*STRIDE_I + jpp + k*STRIDE_K]
                    ...
                    // Local acceleration
                    if (k == 0) {
                        alc[k] =  accn_amp * sin(2.0 * Pi * F * time) * x[STRIDE_K + i*STRIDE_I + j];
                    } else {
                        alc[k] = -accn_amp * sin(2.0 * Pi * F * time) * x[i*STRIDE_I + j];
                    }
            ...
            for (int i = 0; i < n[0]; i++) {
                for (int jj = 1; jj < n[1]; jj++) {
                    ...
                    double ca = dxix[idx] * u[i*STRIDE_I + jj] * fabs(ajac[idx])
                            + dxix[idx_jm1] * u[i*STRIDE_I + (jj-1)] * fabs(ajac[idx_jm1]);
                    double cb = dxiy[idx] * u[i*STRIDE_I + jj + STRIDE_K] * fabs(ajac[idx])
                            + dxiy[idx_jm1] * u[i*STRIDE_I + (jj-1) + STRIDE_K] * fabs(ajac[idx_jm1]);
            ...
            for (int i = 0; i < n[0] - 1; i++) {
                ...
                for (int jj = 1; jj < n[1] - 1; jj++) {
                    ...
                    dil[idx] = dxix[idx] * (u[ipp_j] - u[inn_j]) / (2.0*dxi[0])
                            + dex[idx] * (u[i_jpp] - u[i_jnn]) / (2.0*dxi[1])
                            + dey[idx] * (u[i*STRIDE_I + jpp + STRIDE_K] - u[i*STRIDE_I + jnn + STRIDE_K]) / (2.0*dxi[1])
                            + dxiy[idx] * (u[ipp_j + STRIDE_K] - u[inn_j + STRIDE_K]) / (2.0*dxi[0]);
                    ...
                    if (dil[idx] > dmax_local) dmax_local = dil[idx];
                }
            ...
            for (int i = 0; i < n[0] - 1; i++) {
                for (int jj = 0; jj < n[1]; jj += n[1] - 1) {
                    ...
                    double dv_det;
                    if (jj == 0) 
                        dv_det = 1.0 / dxi[1] * (u[i*STRIDE_I + jpp + STRIDE_K] - u[i*STRIDE_I + jj + STRIDE_K]);
                    else // jj == n[1]-1
                        dv_det = 1.0 / dxi[1] * (u[i*STRIDE_I + jj + STRIDE_K] - u[i*STRIDE_I + jnn + STRIDE_K]);
                    ...
                    double du_det;
                    if (jj == 0)        
                        du_det = 1.0 / dxi[1] * (u[i*STRIDE_I + jpp] - u[i*STRIDE_I + jj]);
                    else // jj == n[1]-1
                        du_det = 1.0 / dxi[1] * (u[i*STRIDE_I + jj] - u[i*STRIDE_I + jnn]);
    ...
    void sip9p(double *ap, double *ae, double *as, double *an, 
        ...
        // Precompute neighbor indices for all i values
        int *inn_arr = new int[n[0]];
        int *ipp_arr = new int[n[0]];
        
        #pragma omp parallel for schedule(static)
        for (int i = 0; i < n[0]-1; i++) {
            inn_arr[i] = (i == 0) ? n[0]-2 : i-1;
            ipp_arr[i] = i+1;
        }
        
        // Forward elimination - compute L and U matrices (only once)
        // Keep this serial due to complex dependencies
        for (int i = 0; i < n[0]-1; i++) {
            int row_base = i * STRIDE_I;
            int inn = inn_arr[i];
            int row_inn = inn * STRIDE_I;
            int row_last = (n[0]-1) * STRIDE_I;
            ...
            for (int j = 1; j < n[1]-1; j++) {
                ...
                double alp_bn_inn = alp * bn[row_inn + j];
                double alp_be_prev = alp * be[idx - 1];
                
                bw[idx] = (aw[idx] + alp*anw[idx] - bsw[idx]*bn[row_inn + j - 1]) / 
                        (1.0 + alp_bn_inn);
                
                bs[idx] = (as[idx] + alp*ase[idx] - bsw[idx]*be[row_inn + j - 1]) / 
                        (1.0 + alp_be_prev);
                
                double ad = anw[idx] + ase[idx] - bs[idx]*be[idx - 1] - bw[idx]*bn[row_inn + j];
                
                bp[idx] = ap[idx] - alp*ad - bs[idx]*bn[idx - 1] - bw[idx]*be[row_inn + j] - 
                        bsw[idx]*bne[row_inn + j - 1];
                
                double inv_bp = 1.0 / bp[idx];
                
                bn[idx] = (an[idx] + alp*anw[idx] - alp_bn_inn*bw[idx] - 
                        bw[idx]*bne[row_inn + j]) * inv_bp;
                
                be[idx] = (ae[idx] + alp*ase[idx] - alp_be_prev*bs[idx] - 
                ...
                if (i == 0) {
                    int last_idx = row_last + j;
                    bsw[last_idx] = bsw[idx];
                    bn[last_idx] = bn[idx];
                    bs[last_idx] = bs[idx];
                    bne[last_idx] = bne[idx];
                    be[last_idx] = be[idx];
                    bw[last_idx] = bw[idx];
                    bp[last_idx] = bp[idx];
                }
        ...
        for (int iter = 0; iter < maxiter; iter++) {
            ...
            for (int i = 0; i < n[0]-1; i++) {
                ...
                for (int j = 1; j < n[1]-1; j++) {
                    ...
                    double res = q[idx] - ap[idx]*phi[idx] - 
                            ae[idx]*phi[row_ipp + j] - an[idx]*phi[idx + 1] - 
                            as[idx]*phi[idx - 1] - aw[idx]*phi[row_inn + j] - 
                            anw[idx]*phi[row_inn + j + 1] - ane[idx]*phi[row_ipp + j + 1] - 
                            asw[idx]*phi[row_inn + j - 1] - ase[idx]*phi[row_ipp + j - 1];
                    
                    ssum += fabs(res);
                    
                    // Forward substitution
                    qp[idx] = (res - bs[idx]*qp[idx - 1] - 
                            bw[idx]*qp[row_inn + j] - bsw[idx]*qp[row_inn + j - 1]) / bp[idx];
                    
                    // Handle periodic boundary
                    if (i == 0) {
                        qp[row_last + j] = qp[idx];
                    }
                }
            }
            ...
            for (int i = n[0]-2; i >= 0; i--) {
                ...
                for (int j = n[1]-2; j >= 1; j--) {
                    ...
                    del[idx] = qp[idx] - bn[idx]*del[idx + 1] - 
                            be[idx]*del[row_ipp + j] - bne[idx]*del[row_ipp + j + 1];
                    
                    phi[idx] += del[idx];
                    
                    // Handle periodic boundary
                    if (is_periodic) {
                        phi[row_last + j] = phi[idx];
                    }
                }
            }
    ...
    void gauss(double *ap, double *ae, double *as, double *an, 
        ...
        for (int iter = 0; iter < maxiter; iter++) {
            ...
            {
                // Each thread processes a range of i values
            ...
            // Compute sumnor only on first iteration (serial section)
            if (iter == 0) {
                sumnor = (ssum != 0.0) ? ssum : 1.0;
            }
...
int main() {
    ...
    cout << "Wall time: " << duration.count() << " ms\n" << endl;
